...
def Run(
  ...
  for i in range(7):
    ...
    codeTestSetIter[i]["C1"] = "code"
    codeTestSetIter[i]["D1"] = "output"
    codeTestSetIter[i]["E1"] = "episode length"
  ...
  for sheetIndex in range(7):
    ...
    for i in range(20):
      ...
      while episodeLength < 5:
        ...
        codeTestSetIter[sheetIndex]["C" + str(i + 2)] = code
        ...
        codeTestSetIter[sheetIndex]["E"+str(i+2)] = episodeLength

        # Check if code passed
        if not traceback:
          print(episodeLength)
          codeTestSetIter[sheetIndex]["F"+str(i+2)] = "Y"
          del stdout, generatedCommand
          gc.collect()
          break
        ...
        if episodeLength == 5:
          print("Failed")
          codeTestSetIter[sheetIndex]["F"+str(i+2)] = "N"
          del stdout, generatedCommand
          gc.collect()
          break
        else:
          # Restructure prompt for next iteration
          if RAGContext == "":
            prompt = util.ragWrongCodePromptTemplateWithoutContext.format(
              question=codeTestSetIter[sheetIndex]["B"+str(i+2)].value,
              system_prompt=util.systemPrompt,
          ...
          # Clean up memory
          del generatedCommand, stdout
          gc.collect()
      modelName_ = modelName.split("/")[-1] if modelName != "OpenROAD-Assistant/Script_Adaptor" else "OpenROAD-Assistant"
      codeTestSet.save(resultPath+modelName_+"-iterate.xlsx")
